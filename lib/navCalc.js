...
// Longitude-to-time conversion (15° = 1 hour, standard LMT relationship —
// east is later, west is earlier) plus International Date Line crossing:
// detects whether the shortest path between the two points actually
// crosses the ±180° meridian, and if so whether a day is gained (crossing
// westbound) or lost (crossing eastbound) — the classic syllabus rule.
export function dateLineInfo(lat1, lon1, lat2, lon2) {
  const chLong = lonDiff(lon1, lon2);
  const timeDiffHours = chLong / 15;
  const unwrapped2 = lon1 + chLong; // where point B "really" is on a continuous line from A
  const crossesIDL = unwrapped2 > 180 || unwrapped2 < -180;
  let dayChange = 0;
  let direction = null;
  if (crossesIDL) {
    if (chLong > 0) {
      dayChange = -1; // eastbound across the IDL: subtract a day
      direction = "east";
    } else {
      dayChange = 1; // westbound across the IDL: add a day
      direction = "west";
    }
  }
  return { chLong, timeDiffHours, crossesIDL, dayChange, direction };
}
